...
var Popper = _interopDefault(require('vue-popperjs'));
  ...
  computed: {
    earliestArrival: function earliestArrival() {
      if (this.selectedDate1) {
        var parsed = parse(this.selectedDate1, 'YYYY-MM-DD', new Date());
    ...
    earliestDeparture: function earliestDeparture() {
      if (this.selectedDate2) {
        var parsed = parse(this.selectedDate2, 'YYYY-MM-DD', new Date());
        return format(parsed, 'DD.MM.YYYY')
      }      
      return null
    },
